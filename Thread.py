...
def get_avg_stars_from_players():
    response = requests.get('https://api.clashofclans.com/v1/clans/%232Y00GGVCU/currentwar/leaguegroup',headers=headers)
    war_tag_json = response.json()
    # print(war_tag_json['rounds'])
    my_dict = {}
...
def combine():
    ...
    first_list = pool.submit(get_cwlinfo_from_war_tag)
    second_list = pool.submit(get_avg_stars_from_players)
    third_list = pool.submit(get_ratio_of_attacks)
    new_list = []

    for distance,stars,attackRatio in zip(first_list.result(),second_list.result(),third_list.result()):
      #Name, mapPosDiff, avgStars, strWarAttacks, warAttackRatio
